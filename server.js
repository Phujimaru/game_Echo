...
function applyBardSong(p, pattern, targets) {
  ...
  switch (pattern) {
    ...
    case "RJR": // Fate's Prelude: โชคลาภในการจั่วไพ่ครั้งถัดไป (ซ้อนทับได้สูงสุด 3 — patch 2.0.6.1)
      ...
      break;
    case "JRR": // Rejuvenation (patch 2.0.6.1): HP +1 / เกราะ +1 / ดาเมจครั้งต่อไป +1 (ไม่ซ้อนทับ)
      if (!t) return;
      healHp(t, 1);
      t.armor = Math.min(maxArmorOf(t), t.armor + 1);
      t.statuses.empower = 1; // เสริมพลัง: การโจมตีครั้งถัดไป +1 (ไม่ซ้อนทับ — หมดเมื่อได้โจมตี)
      lastLog.push(`🎼💖 Rejuvenation — ${t.name} ฟื้น HP +1 เกราะ +1 และการโจมตีครั้งถัดไป +1 (ไม่ซ้อนทับ)`);
      break;
...
function maybeBardDim(p, live) {
  ...
  if (kind === "bloodDim") {
    ...
    p.statuses.fortune = Math.min(BARD_FORTUNE_MAX, (p.statuses.fortune || 0) + BARD_BLOOD_FORTUNE);
    p.statuses.evade = Math.min(BARD_EVADE_MAX, (p.statuses.evade || 0) + BARD_BLOOD_EVADE);
    lastLog.push(`❤️🌅 ${p.name} เปิดมิติมายาบรรเลงโลหิต! (3 เทิร์น — นับเป็นตอนเช้า) กดโน้ตได้สูงสุด ${BARD_DIM_NOTES_PER_TURN} ครั้งต่อเทิร์น ต้านสถานะผิดปกติ / ทุกคนติดขัดแย้ง +1 ดาเมจ 3 เทิร์น / คีตกวีได้โชคลาภ ${BARD_BLOOD_FORTUNE} ครั้ง และหลบหลีก ${BARD_BLOOD_EVADE} ครั้ง`);
  } else {
...
const BARD_BLOOD_FORTUNE = 3;       // มิติโลหิต: คีตกวีได้โชคลาภในการจั่วไพ่ 3 ครั้ง (patch 2.0.6.1 — ลดจาก 5)
const BARD_BLOOD_EVADE = 3;         // มิติโลหิต: คีตกวีได้หลบหลีก 3 ครั้ง (patch 2.0.6.1)
...
const SHIKI_WITHER_LINE_MAX = 6; // โหมดท่าไม้ตาย 2: เส้นชีวิตสะสมได้สูงสุด 6 หน่วยต่อคน (3 จากแหล่งปกติ + 3 จากท่าไม้ตาย = 60%)
...
function dealRound() {
  ...
  // ความตายที่โรยรา (ชิกิ patch 2.0.6.1): ทุกเทิร์นที่ท่าไม้ตายยังทำงาน มอบเส้นชีวิต +1
  //  ให้ผู้เล่นทุกคนยกเว้นตัวเอง — ท่าไม้ตายแจกได้สูงสุด 3 หน่วยต่อคน (หยุดแจกหลังเทิร์นที่ 3)
  //  รวมกับแหล่งปกติ (สูงสุด 3) = เพดาน 6 หน่วย (60%)
...
function useSkill(id, tier, targets, item) {
  ...
  if (isShikiLifeline && shikiLifelineTarget) {
    ...
    p.statuses.godslay = SHIKI_GODSLAY_TURNS + 1; // 2 เทิร์น (+1 ชดเชยการลดสถานะตอนจบเทิร์น) — ไม่สะสม
  ...
  if (st === "wither") {
    ...
    lastLog.push(`🥀 ${p.name} ความตายที่โรยรา — ความตายเริ่มโรยราลงบนสนาม 5 เทิร์น! (โจมตีปกติมีโอกาสสังหาร 10% ต่อเส้นชีวิต 1 หน่วย — ท่าไม้ตายแจกเส้นชีวิตได้สูงสุด ${SHIKI_WITHER_ADD_MAX} หน่วยต่อคน)`);
  }
...
function doAttack(byId, targetId) {
  ...
  // หลบหลีก (Encore / มิติโลหิต — Bard): หลบการโดนโจมตี 100% — ซ้อนทับได้ หมดไปทีละ 1 ครั้ง (patch 2.0.6.1)
  if ((target.statuses.evade || 0) > 0) {
    ...
    lastLog.push(`💨 หลบหลีก! ${target.name} หลบการโจมตีของ ${attacker.name} ได้ (100%) — เหลือหลบหลีกอีก ${target.statuses.evade || 0} ครั้ง`);
    lastAttack = {
  ...
  if (target.characterId === "shiki" && (target.statuses.godslay || 0) > 0) {
    ...
    if (ultKey) {
      ...
      const ultImg = (TRANSFORMS[ultKey] && TRANSFORMS[ultKey].img) || displayImg(attacker);
  ...
  let base = 1 + oberonZero + (veilAtk ? 1 : 0) + (empowerAtk ? 1 : 0) + (ginga ? 1 : 0) + (beam ? 2 : 0) + (lastStanding ? 1 : 0) + ohgerBonus + (humanityAtk ? 4 : 0) + (spearAtk ? 1 : 0) + profitAtk + appleAtk + (tigerAtk ? 1 : 0) + (partnerAtk ? 1 : 0) + pigDmg + aquaAtk + shradeAtk; // Beam Magnum +2
