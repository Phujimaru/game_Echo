...
const HAKUNO_MOONCELL_ATK_FEMALE = 2; // MOON*CELL: ร่างหญิง พลังโจมตี +2 (รวมเป็น 3)
const HAKUNO_MOONCELL_ATK_MALE = 1;   // MOON*CELL: ร่างชาย พลังโจมตีเพิ่มอีก +1 (ซ้อนกับ +1 ถาวร รวมเป็น 3)
...
function doAttack(byId, targetId) {
  ...
  // ---------- คิชินามิ ฮาคุโนะ (patch 2.2.1) ----------
  const hakunoMaleAtk = (attacker.characterId === "hakuno" && attacker.hakunoGender === "male") ? HAKUNO_MALE_ATK_BONUS : 0; // ร่างชาย: พลังโจมตีถาวร +1
  const hakunoMoonAtk = (attacker.characterId === "hakuno" && (attacker.statuses.moonCell || 0) > 0)
    ? (attacker.hakunoGender === "female" ? HAKUNO_MOONCELL_ATK_FEMALE : HAKUNO_MOONCELL_ATK_MALE)
    : 0; // MOON*CELL: ร่างหญิง +2 / ร่างชาย +1 เพิ่มเติม (รวมเป็น 3 ทั้งคู่)
