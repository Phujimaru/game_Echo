...
const TRANSFORMS = {
  ...
  // ---------- ซาโตรุ อาเคฟุ (patch 2.0.8.2) ----------
  // wonderofu: ท่าไม้ตายอัตโนมัติ Wonder of U — วีดีโอ 23.8 วิ + เพลงเล่นค้างตราบใดที่มีคนติด [Calamity]
  wonderofu: { img: "/characters/satoru/wonderofu.png", video: "/characters/satoru/wonderofu.mp4", title: "WONDER OF U", label: "ปล่อยท่าไม้ตาย", seconds: 24, music: "wonderofu", afterReveal: false },
...
function useSkill(id, tier, targets, item) {
  ...
  if (isAshenTrail) {
    ...
    lastLog.push(`🐴🔥 ${p.name} Ashen Trail: Cinderella Gray — ใช้ Stamina ${OGURI_ULT2_STAMINA} + ยุคทอง ${OGURI_GOLD_MAX}! ทุกคนถูกบังคับจั่วเพิ่ม ${OGURI_ASHEN_DRAWS} ใบ และห้ามใช้สกิลตลอดเทิร์นนี้${bustedNames.length ? ` — ${bustedNames.join(", ")} ไพ่แตก!` : ""}`);
...
function doAttack(byId, targetId) {
  ...
  let base = 1 + oberonZero + (veilAtk ? 1 : 0) + (empowerAtk ? 1 : 0) + (ginga ? 1 : 0) + (beam ? 2 : 0) + (lastStanding ? 1 : 0) + ohgerBonus + (humanityAtk ? 4 : 0) + (spearAtk ? 1 : 0) + profitAtk + appleAtk + (tigerAtk ? 1 : 0) + (partnerAtk ? 1 : 0) + pigDmg + aquaAtk + shradeAtk + oguriGoldAtk + (victoryAtk ? 1 : 0) + (ashenAtk ? 1 : 0); // Beam Magnum +2
