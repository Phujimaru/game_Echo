...
const BASIC_DEBUFF_CLEAR = ["discord", "sleep", "kstun", "stun", "nodraw", "noskill", "aquapoison", "weak", "fragile", "spellburden", "oblada", "calamity", "hburn", "phenexBanUlt", "nanayaSeal"];
...
// สกิลติดตัวถูก "อันนี้ของนายรึเปล่า" ปิดใช้งานอยู่ไหม (ใช้เช็คก่อนให้สกิลติดตัวของตัวละครอื่นทำงาน)
function passiveSealed(p) {
  return !!p && ((p.statuses && p.statuses.nanayaSeal) || 0) > 0;
}
...
const DEBUFF_KEYS = ["discord", "sleep", "kstun", "stun", "nodraw", "noskill", "sena",
  ...
  "oblada", "calamity", "hburn", "phenexBanUlt", "nanayaSeal"];
...
function useSkill(id, tier, targets, item) {
  ...
  if (p.skillUsedRound && !mageRepeat && !gambleRepeat && !isApplePick && !isAquaLeader && !isTohnoPick) return; // ใช้สกิลได้เพียง 1 อันต่อเทิร์น (ซ้ำ/ซ้อนไม่ได้)
  ...
  if (!isApplePick && !isAquaLeader && !isTohnoPick) p.skillUsedRound = true; // เอาแบบนี้ได้ไหม / เปลี่ยนหัวหน้า / มีดพับประจำตระกูล: ไม่นับเป็นการใช้สกิลของเทิร์น
...
function afterNanayaAttack(attacker) {
...
function doAttack(byId, targetId) {
  ...
  if (attacker.characterId === "tohno" && tohnoLevel >= 2 && !passiveSealed(attacker)) {
    const chance = TOHNO_KILL_CHANCE[tohnoLevel];
  ...
  attacker.nanayaMissedThisAttack = false;
  if (attacker.characterId === "nanaya" && attacker.nanayaEyeOn && !passiveSealed(attacker)) {
    if (Math.random() < NANAYA_KILL_CHANCE) {
    ...
    attacker.nanayaMissedThisAttack = true; // หัวใจฆาตกร: เปิดโอกาสโจมตีซ้ำทันที
  ...
  const showAttackFx = () => {
    ...
    startPhaseTimer(fxSkills.length ? ATTACKFX_TIME + 2 : ATTACKFX_TIME, () => runCutsceneQueue(() => afterNanayaAttack(attacker)));
    broadcastState();
  };
  ...
  if ((beamPlusAtk || (beam && attacker.characterId === "banagher") || unibeam2Atk || storiumAtk || phenexPurgeAtk) && cutsceneQueue.length) runCutsceneQueue(showAttackFx);
  else showAttackFx();
}
