...
const HIDDEN_UNTIL_REVEAL = ["beam", "ohger", "absorb", "spear", "nightmare", "beamplus"];
...
function doAttack(byId, targetId) {
  ...
  lastAttack = {
    byName: attacker.name, byImg: displayImg(attacker), byColor: POSITION_COLORS[attacker.position] || "#888",
    targetName: target.name, targetImg: displayImg(target), targetColor: POSITION_COLORS[target.position] || "#888",
    dmg, aoe: ginga || (attacker.statuses.solarburst || 0) > 0 || beamPlusAtk, revenge: isRevenge, skills: fxSkills,
  };
  const showAttackFx = () => {
    gameState = "ATTACKING";
  ...
  // Beam Magnum Plus (ริดดี้ patch 2.1.1): เล่นวีดีโอ riddheBeam ที่ค้างคิวก่อน แล้วค่อยขึ้นสรุปความเสียหาย
  //  (ปกติทุกท่าอื่นจะขึ้นสรุปความเสียหายก่อนแล้วค่อยเล่นวีดีโอค้างคิวตอนจบ — ท่านี้กลับลำดับเฉพาะตัว)
  if (beamPlusAtk && cutsceneQueue.length) runCutsceneQueue(showAttackFx);
  else showAttackFx();
}
