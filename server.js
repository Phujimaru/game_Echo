...
function useSkill(id, tier, targets, item) {
  ...
  // บานาจ (patch 2.1.2): แสงที่ไม่อยู่เพียงลำพัง — ต้องมีกระสุน Beam Magnum เหลืออย่างน้อย 1 นัดทั้งคู่ (ตัวเอง + ริดดี้พันธมิตร)
  if (st === "unibeam2") {
    const rAlly = riddheAllied(p);
    if (!rAlly || (p.beamAmmo || 0) <= 0 || (rAlly.beamAmmo || 0) <= 0) return;
  }
