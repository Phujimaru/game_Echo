...
function applyBardSong(p, pattern, targets) {
  ...
  switch (pattern) {
    ...
    case "RJR": // Fate's Prelude: โชคลาภในการจั่วไพ่ครั้งถัดไป
      ...
      lastLog.push(`🎼🍀 Fate's Prelude — ${t.name} ได้รับโชคลาภในการจั่วไพ่ครั้งถัดไป`);
      break;
...
function dealRound() {
  ...
  for (const p of Object.values(players)) {
    ...
    if (p.overworkNext) {
      ...
      p.armor = 0;
      p.shield = 0;
      lastLog.push(`🥵 ${p.name} ติดสถานะ [โหมงานหนัก] 3 เทิร์น — เกราะพังทั้งหมด ฟื้นเกราะไม่ได้ ใช้แต้มสกิลเพิ่ม 1 และเสี่ยงสตั้น 20% ทุกเทิร์น`);
    }
  ...
  // มิติมายาบรรเลงโลหิต (Bard): ผู้เล่นทุกคนฟื้นพลังงาน +1 ทุกเทิร์นที่มิติเปิดอยู่
  if (alivePlayers().some((b) => b.characterId === "bard" && (b.statuses.bloodDim || 0) > 0)) {
    for (const p of alivePlayers()) addSkill(p, 1);
    lastLog.push("❤️🌅 มิติมายาบรรเลงโลหิต — ผู้เล่นทุกคนฟื้นพลังงาน +1");
  }
...
function hit(id) {
  ...
  // โชคลาภ (Fate's Prelude — Bard): การจั่วครั้งถัดไปได้ใบที่ดีที่สุดที่ไม่ทำให้แตก (หมดผลเมื่อใช้)
